...
vim.keymap.set('n', '<C-\\>', '<cmd>:vs<cr>', { desc = 'Open a window to the right' })
...
require('lazy').setup {
  ...
  { -- Autoformat
    ...
    -- This gives a desiginated button for formatting, but I'm okay with format
    -- on save for now.
    -- keys = {
    --   {
    --     '<leader>f',
      ...
      ls.add_snippets(nil, {
        ...
        all = {
          snip({
            trig = 'ts',
            namr = 'TimeStamp',
            dscr = 'Time stamp in the form of HH:MM',
          }, {
            func(date_raw, {}),
          }),
        },
...
-- NOTE: Setup None_ls for formatting
